...
\author{Walter Daelemans* \and Jakub Zavrel*$\dagger$ \and Ko van der Sloot \and
	...
	http://ilk.uvt.nl/downloads/pub/papers/ilk.0703.pdf. All rights reserved
...
{\bf version 6.0} \\ \vspace*{0.5cm}{\huge Reference Guide}\\
\vspace*{1cm} {\it ILK Technical Report -- ILK 07-03}}
...
Knowledge) research group in 1997, and with the increasing use of {\sc
  mbl} at the CNTS research group of the University of Antwerp, the
need for a well-coded and uniform tool for our main algorithms became
more urgent. TiMBL was the result of combining ideas from a number of
...
other domains where classification tasks are learned from
examples. With the release of the sixth version of TiMBL, we now
proudly release our software under the GPL license, for anyone to use
...
The current release (version 6.0) succeeds major release 5.0 and minor
...
efficiency improvements; 6.0 adds another large batch of improvements
in memory usage and speed. Also on the outside, version 6.0 offers
several new metrics, verbosity options, and other command-line
switches. An elaborate description of the changes from version 1.0 up
to 6.0 can be found in Chapter~\ref{changes}. Although all new
features have been tested for some time in our research groups, the
software may still contain bugs and inconsistencies in some places. We
would appreciate it if you would send bug reports, ideas about
enhancements of the software and the manual, and any other comments
you might have, to {\tt Timbl@uvt.nl}.
...
In publication of research that makes use of TiMBL 6.0, a citation
should be given of: {\em ``Walter Daelemans, Jakub Zavrel, Ko van der
  ...
  Learner, version 6.0, Reference Guide. ILK Technical Report 07-03,
  Available from \\ {\tt
    http://ilk.uvt.nl/downloads/pub/papers/ilk0703.pdf}''}

For information about commercial licenses for TiMBL 6.0,
...
Following the links from that page, you can download the file {\tt
  Timbl.6.0.tar.gz}. This file contains the complete source code (C++)
...
{\tt > gunzip Timbl.6.0.tar.gz}

and unpack the tar archive:

{\tt > tar -xvf Timbl.6.0.tar}

This will make a directory {\tt Timbl6} under your current
...
3.2.x -- 4.1.x}, assuming that {\tt make} is actually {\tt
...
Within in the {\tt Timbl6} directory two subdirectories are also
...
released versions (1.0 up to 6.0) for users already familiar with the
program.
