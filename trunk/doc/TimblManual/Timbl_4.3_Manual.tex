...
\begin{itemize} 
	\item the {\tt -d metriccode} option sets the distance 
        ...
        \item the {\tt -L n} option sets the frequency threshold in
              the optional switch (backoff) from {\sc mvdm} to
              Overlap; whenever in an {\sc mvdm} distance computation
              one or both of a pair of values occur less than {\tt n}
              times, Overlap is used rather than the {\sc mvdm}
              metric.  The default value for {\tt n} is 1 (no
              switching).
...
from this manual. This new API, describing the underlying structure of
TiMBL (which has changed considerably from version 3.0 to 4.0-4.3), is
available as a separate document in the TiMBL software distribution
from TiMBL version 4.2 onwards.
