...
examples. With the release of the sixth version of TiMBL, we have
moved to releasing our software under the GPL license, for anyone to
use under the conditions stated in the license.
...
scrutiny by all past and present members of the ILK and CLiPS (formerly CNTS)
groups in Tilburg and Antwerp. We are furthermore indebted to Ton
...
Faculty of Humanities of Tilburg University. NWO funding
...
The current release (version 6.2) succeeds major release 6.0 and minor update 6.1. It provides several new distance functions, and the internal code has been overhauled significantly. Version 6.1 introduced, among other software improvements, autoconfiguration.  An
elaborate description of the changes from version 1.0 up to 6.2 can be
...
In publication of research that makes use of TiMBL 6.2, a citation
...
Following the links from that page, you can download the file {\tt
  timbl-6.2.tar.gz}. This file contains the complete source code (C++)
...
If you don't use the {\tt --prefix} option, TiMBL will try to install itself
...
also created: {\tt share/doc/timbl} where the TiMBL 6.2 documentation
...
If you did not install TiMBL, the executable can be found in the {\tt
  src} directory of the build. The demo files can be found in the {\tt demo}
directory.
...
released versions (1.0 up to 6.2) for users already familiar with the
program.

\section{From version 6.1 to 6.2}

The {\sc IGTree} algorithm has been optimized beyond the optimizations introduced in version 6.0. With very large training sets, {\sc IGTree} was reported to be almost exponentially slower in the later stages of training. This has now been resolved; trees are now built in near-linear time.

Finally, besides minor bug fixes, many internal changes were made to make the code better maintainable and extendible.
